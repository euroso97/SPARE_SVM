...
def run_spare_ba_train():

    csv_path = '/home/kylebaik/Test_Projects/NiChart_Paper_SPARE_Experiments/lists/SPARE-BA-Harmonized.csv'

    try:
        print(f"\nAttempting to load data from {csv_path} using load_tabular_data...")
        features_df_loaded, labels_series_loaded = load_tabular_data(csv_path, 'Age', ignore_columns=['MRID', 'Sex', 'Study', 'SITE'])
    ...
    svm_trainer_linear = SVMTrainer(task = 'regression', kernel='linear', tune_hyperparameters=True, random_state=42)
    print("Training Linear SVM model...")
    svm_trainer_linear.train(features_df_loaded, labels_series_loaded)
    
    print("Linear SVM Details:", svm_trainer_linear.get_model_details())
    # Save the linear model
    svm_trainer_linear.save_model('/home/kylebaik/Projects/NiChart_SPARE/models/SPARE_BA/spare_ba_svm.joblib')
